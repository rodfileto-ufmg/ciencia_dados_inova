...
plot(net_doc, layout = l, vertex.label = NA,
     vertex.color = adjustcolor(V(net_doc)$color_science, alpha.f = .8), vertex.size = 15, vertex.frame.color = "#FF000000")
# dev.off()
...
# ##### cluster similarity based on textual distance - BM 25 Score ####
# 
...
rstudioapi::terminalExecute(glue::glue("VOSviewer.exe -map {normalizePath(getwd())}//03_cienciometria/dados/map_clusters.txt -network {normalizePath(getwd())}/03_cienciometria/dados/network_clusters.txt -largest_component -attraction 4 -repulsion -1"))
